...
class LexaDB {
  ...
  async getNote(id: string): Promise<DbNote | undefined> {
...
const dom = {
  ...
  sidebarCloseBtn:byId<HTMLButtonElement>('close-sidebar'),
  ...
  closeBookBtn:   byId<HTMLButtonElement>('close-sidebar'),
  ...
  prevPage:       byId<HTMLButtonElement>('prev-btn'),
  nextPage:       byId<HTMLButtonElement>('next-btn'),
  pageInput:      byId<HTMLInputElement>('page-input'),
  totalPages:     byId('total-pages'),
  zoomCtrl:       byId('controls-sidebar'),
  zoomIn:         byId<HTMLButtonElement>('zoom-in-btn'),
  zoomOut:        byId<HTMLButtonElement>('zoom-out-btn'),
  zoomVal:        byId('zoom-level'),
  fitPage:        byId<HTMLButtonElement>('fullscreen-btn'),
  ...
  voiceSel:       byId<HTMLSelectElement>('voice-select'),
  playBtn:        byId<HTMLButtonElement>('play-btn'),
  stopBtn:        byId<HTMLButtonElement>('play-btn'),
  skipBack:       byId<HTMLButtonElement>('prev-btn'),
  skipFwd:        byId<HTMLButtonElement>('next-btn'),
  spdDn:          byId<HTMLButtonElement>('speed-slider'),
  spdUp:          byId<HTMLButtonElement>('speed-slider'),
  spdVal:         byId('speed-display'),
  pitchSlider:    byId<HTMLInputElement>('pitch-slider'),
  autoBtn:        byId<HTMLButtonElement>('auto-advance'),
  progFill:       byId('progress-fill'),
  progLbl:        byId('current-page'),
  progPct:        byId('total-pages'),
  swipeHint:      byId('main-content'),
  // New
  fullScreenBtn:  byId<HTMLButtonElement>('fullscreen-btn'),
  notePrompt:     byId('notePrompt'),
  addNoteBtn:     byId<HTMLButtonElement>('addNoteBtn'),
  navLibList:     byId('navLibList'),
  navNoteList:    byId('navNoteList'),
  waterCanvas:    byId<HTMLCanvasElement>('waterCanvas'),
};
...
async function refreshLibrary(): Promise<void> {
  ...
  notes.sort((a: DbNote, b: DbNote) => b.updatedAt - a.updatedAt);
...
function showNoteContent(note: DbNote) {
  let overlay = document.querySelector<HTMLDivElement>('.note-view-overlay');
  ...
  (overlay.querySelector('#noteViewTitle') as HTMLElement).textContent = note.title;
  (overlay.querySelector('#noteViewBody') as HTMLElement).textContent = note.content;
...
function setPlayState(playing: boolean): void {
  (dom.playBtn.querySelector('.play-icon') as HTMLElement).style.display  = playing ? 'none' : '';
  (dom.playBtn.querySelector('.pause-icon') as HTMLElement).style.display = playing ? '' : 'none';
}
...
(function initWaterRipple(): void {
  ...
  let rippleData: Int16Array, lastRippleData: Int16Array;
  let rippleWidth: number, rippleHeight: number;
  let size: number;
    ...
    for (let i = rippleWidth; i < size - rippleWidth; i++) {
      rippleData[i] = (
        (lastRippleData[i - 1] +
          ...
          lastRippleData[i + rippleWidth]) >> 1
      ) - rippleData[i];
...
(function init4D(): void {
  ...
  const grid    = welcome.querySelector<HTMLElement>('.cyber-grid');
  const pctEl   = byId<HTMLElement>('initPct');
  if (!target) return;
